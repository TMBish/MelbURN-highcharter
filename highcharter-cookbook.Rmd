...
Layering series onto the same chart works much like ggplot. It's a little bet less natural as the data object must be supplied each time which prevents big long data into highcharter chanins but it might look something like this:

```{r}
# gamelogs %>%
#   filter(namePlayer == "Andre Drummond") %>%
#   arrange(numberGamePlayerSeason) %>%
#   hchart("column", hcaes(x = numberGamePlayerSeason, y = dreb)) %>%
#   hc_add_series(data = NULL, "column", hcaes(x = numberGamePlayerSeason, y = oreb)) %>%
...
Starting simply using a couple of the simple boolean chart options we see there we can adjust the `gridLineWidth` to 10 it will make the major grid lines to be massive and change the dash style to "shortdash". We know we can enter thestring "shortdash" as a possible option for the grid line dash style because when we click on that option in the API documentation it says so:
...
So it worked, but given we only passed the chart the assist / game data series it doesn't know what players the values correspond to or anything. We can add these to the chart by specifying axis "categories" manually:
...
So this option suggests you can provide the data as an array of x, y pairs. This isn't a perfectly natural data structure for R but we can construct a list of x, y pairs from a dataframe with the help of the `list_parse2()` function exported with highcharter.
...
Notice that the x values haven't been placed on the x Axis as we'd like but rather appear as the "name" of the column when you hover over it - this is called the series name.

Instead of manually we can construct the same list structure for the top 10 players using `list_parse2()`. This function will create a list of length equal to the rows in your dataframe with pairs of values that come from the columns:
...
OK that's what we were looking for let's pass it as the value to the data argument and add the names on as categories to finish it off
...
To achieve this list structure we use the other list parsing function `list_parse()` which will do the same thing but preserve names. However, we need the column names to match up to the graph dimensions so we first have to rename columns. Best way to understand is to inspect a couple of elements:
...
Hopefully what you should understand after these examples is that highcharts on the javascript side are just giant JSON configuration objects. Even the data is nested within the series slot in the appropriately structured format. On the R side a set of functions and helpers enable you to create this JSON often with only 1 or 2 lines of code. Most of the time you simply pass hchart() a tidy dataframe and let it convert your data into the JSON that will be placed inside the series config. Every now and then, though, and when you need to venture into more complicated chart types (animated charts for example) the data specification won't be simple and you'll need to build the raw data spec list yourself. To do so you need to know how to wrangle lists, likely with purrr, but the most important thing is understanding the structure javascript API needs.
...
gamelogs %>%
  ...
  hc_chart(
    backgroundColor = list(
       linearGradient = c(0, 1, 1, 0),
       stops = list(
         list(0, '#FFFFFF'), # Start Color
         list(0.5, "rgba(253, 185, 39, 0.2)"), # Middle Color
         list(1, "rgba(253, 185, 39, 1)") # End Color
       )
    )
  )
```

Let's make a slightly different lakers theme with a few more color specifications on the axis labels and titles, and a font which is closer to the lakers font:
...
Similarly we could add a picture to the plot area to give the viewer an even faster recognition of what player this chart is visualising:
...
Lookin good.. Using the same approach would could simply url link to a relevant gif file as the background image like follows:
...
Instead of setting the entire background image to be a lebron gif or image I could utilise the annotation functionality within highcharts. Add annotations with the specifications at this [link](https://api.highcharts.com/highcharts/annotations). You can add a single annotation with `hc_add_annotation` or multiple with distinct lists with `hc_add_annotations`. Annotation can be shapes, text or image links.

```{r}
baseBronChart %>%
  hc_add_annotation(
    shapeOptions = list(
      src = bronHeadshot # Link to Lebron's headshot from nbastatR
    ),
    shapes = list(
      type = "image",
      width = 180, height = 131, # Correct image aspect ratio for these headshots
      point = list(
        x = 62, xAxis = 0, # Unit 62 on the x axis
        y = 60, yAxis = 0 # Unit 60 on the y axis
      )
    )
)
```
...
These annotations can be easily added and placed programatically (smartly in the plot area etc) and are a great tool for customising your charts.

### Plot Lines and Bands

Plot lines or bands can be a great way to highlight things on the chart you want to call the users attention to. In the above Lebron James points column charts we know Lebron experienced his first significant injury of his career in this season so we should spefically call that out to the viewer as it's an obvious anomoly with the data shown. It makes sense to do that with a plot area in this example. We could also add his season average points to the line as a reference point.
...
baseBronChart = 
  ...
  hc_xAxis(
    ...
    plotBands = list(
        list(
          from = 35, # First game # of injuryt
          to = 51, # Last game misseds
...
Let's rebuild the whole chart and add a reference to these strings as the labels. Now all I need to write in the callback is that I want to return the exact value of the label (`hc_dataLabel`) that I've prebaked in my dplyr chain. Notice also the slight html markup that will display the opponent team in italics.
...
baseBronChart = 
  ...
  hchart(
    ...
    dataLabels = list(
      enabled = TRUE,
      #useHTML = TRUE, # Render the formatter output as HTML
      formatter = JS(
        "
...
A nice additional to this chart might be to add LeBron's season-by-season scoring average in the top right corner as a reference point. We'll need some more data for that so let's get that first.

```{r}
nbastatR::players_careers(players = "LeBron James", modes = "PerGame")

lebronRegSeason = 
  dataPlayerSeasonTotalsRegularSeason %>%
  select(slugSeason, slugTeam, ptsPerGame)
...
#     lebronRegSeason,
#     "pie",
#     hcaes(name = slugSeason, value = ptsPerGame),
#     name = "Season Avg"
#   ) %>%
#   hc_plotOptions(
#     line = list(
...
Dates and datetimes can be a slightly more difficult exercise in highcharter.
...
highcharter has correctly plotted this as a date axis (the tooltip will show a day of the week for example). Highcharts works where it finds the appropriate format for the span of the dates. Those patterns are:
...
So it looks like our chart is plotted for day or week units. We can change the default for this type by customising the unit formats in the `.dateTimeLabelFormats` property of the x axis. The units (number of ticks and labels) are also predefined by the 
...
With date axes it's also natural to add a date filter on the x axis. There are many options but I think the two most natural ones are a navigator which I can add like this:
...
Notve how the axis is dynamic and updates the format to the appropriate units if you zoom in all the way to an individual week.
...
The last thing you might find difficult with this chart type is adding annotations or plot lines.

To work with datetimes we'll need to get a different dataset
